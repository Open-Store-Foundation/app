...
kotlin {
    android {
        ...
        defaultConfig {
            ...
            val _versionCode =
                rootProject.findProperty("versionCode")?.toString()?.toInt() ?: 1000000000
        ...
        signingConfigs {
            if (props.keys.contains("foundation.openstore.store.key.debug.keystore")) {
                getByName("debug") {
                    storeFile =
                        File(props["foundation.openstore.store.key.debug.keystore"]!!.toString())
                    storePassword =
                        props["foundation.openstore.store.key.debug.keystore_pass"]!!.toString()

                    keyAlias = props["foundation.openstore.store.key.debug.alias"]!!.toString()
                    keyPassword =
                        props["foundation.openstore.store.key.debug.alias_pass"]!!.toString()
                }
            }
            ...
            if (props.keys.contains("foundation.openstore.store.key.release.keystore")) {
                getByName("release") {
                    storeFile =
                        File(props["foundation.openstore.store.key.release.keystore"]!!.toString())
                    storePassword =
                        props["foundation.openstore.store.key.release.keystore_pass"]!!.toString()

                    keyAlias = props["foundation.openstore.store.key.release.alias"]!!.toString()
                    keyPassword =
                        props["foundation.openstore.store.key.release.alias_pass"]!!.toString()
                }
            }
        }
    ...
    dependencies {
        ...
        implementation(projects.apps.store.app.shared)
        implementation(projects.core.log)
    }
}
