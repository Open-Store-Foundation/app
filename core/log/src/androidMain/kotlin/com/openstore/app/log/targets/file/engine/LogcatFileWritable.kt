...
internal class LogcatFileWritable(
    ...
    override fun getFiles(): List<LogFile> = mutableListOf(
        LogFile(fileMain.canonicalPath),
        LogFile(fileSystem.canonicalPath)
    )
    ...
    override fun onInit(config: LoggerConfig) {
        if (isCollecting.compareAndSet(false, true)) {
            runBlocking {
                mutex.withLock {
                    fileMain = getFileByPath(File(config.logsDir.toPath()), TYPE_MAIN)
                    fileSystem = getFileByPath(File(config.logsDir.toPath()), TYPE_SYSTEM)
    ...
    companion object {
        private const val TYPE_MAIN = "commonMain"
